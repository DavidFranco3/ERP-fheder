...
import { cambiaStatusProduccion } from "../../../api/produccion";
import { LogsInformativos } from "../../Logs/LogsSistema/LogsSistema";

function EliminacionLogicaProduccion(props) {
    const { datos, setShowModal, history } = props;
    const { id, folio, planeacion, generalidades, estado } = datos;
    ...
    const onSubmit = e => {
        ...
        try {
            cambiaStatusProduccion(id, dataTemp).then(response => {
                const { data } = response;
                //console.log(data)
                toast.success(data.mensaje);
                LogsInformativos("Se ha actualizado la produccion " + folio, datos);
                setShowModal(false);
                setLoading(false);
                history.push({
                    ...
                    (
                        <>
                            <Alert variant="danger">
                                ...
                                <p className="mensaje">
                                    Esta acción deshabilitara en el sistema la produccion.
                                </p>
                            </Alert>
                        </>
                    ) : (
                        <>
                            <Alert variant="success">
                                ...
                                <p className="mensaje">
                                    Esta acción habilitara en el sistema la produccion.
                                </p>
                            </Alert>
                        </>
                    )
