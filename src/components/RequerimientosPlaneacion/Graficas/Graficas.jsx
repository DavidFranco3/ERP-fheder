...
function Graficas(props) {
    ...
    const cargarDatosProduccion = () => {
        ...
        map(listProgramaProduccion, (ordenes, index) => {
            auxOP.push(ordenes.folio);
            auxProducir.push(ordenes.requerimiento.totalProducir);
        })
    ...
    const cargarDatosCertificado = () => {
        try {
            let auxRes = [], auxRec = [];
            map(listProgramaProduccion, (ordenes, index) => {
                obtenerDatosCertificadoPorOP(ordenes.folio).then(response => {
                    const { data } = response;
